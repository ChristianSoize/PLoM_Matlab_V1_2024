function [ind_step1,ind_step2,ind_step3,ind_step4,ind_step4_task13,ind_step4_task14,ind_step4_task15, ...
    ...
    if ind_step2 == 1
       ...
       alpha_relax2 = 0.3;   % value of alpha_relax (for instance, 0.05) such that alpha_relax(iter >= iter_relax2) = apha_relax2
       ...
       MatRplotHClouds  = [1 2 3          % contains the 3 components numbers of H_ar for which the plot of the clouds are made
                           2 3 6];        % plot nbplotHClouds = 3 clouds
                                         
       MatRplotHpdf =[1 2 3 4 5 6 7 8 9]; % contains the components numbers of H_d and H_ar for which the plot of the pdfs are made
                                          % nbplotHpdf = 9 pdf's are plotted 

       MatRplotHpdf2D = [1 2              % contains the 2 components numbers of H_d and H_ar for which the plot of the joint pdfs are made
                         1 9];            % nbplotHpdf2D = 2 joint pdf's are plotted
    ...
    if ind_step3 == 1
       ...
       %          Indx_obs = [Indq_obs
       %                      n_q + Indw_obs]
       %          In the example below, it is assumed that n_q = 35, n_w = 5, 
       %          Indq_obs = [1 2 3 4 35]', and Indw_obs = [1 5]'
       %          then, Indx_obs = [1 2 3 4 35 36 40]


       MatRplotSamples = [];         % MatRplotSamples(1,nbplotSamples)
                                     % MatRplotSamples = []: no plot, nbplotSamples = 0

       MatRplotClouds = [1 2 3       % MatRplotClouds(nbplotClouds,3): plot the clouds for XX1,XX2,XX13 that is QQ1,QQ2,QQ3  
                         4 36 40];   %                                 plot the clouds for XX4,XX36,XX40 that is QQ4,QQ36,WW5  
                                     % plot, nbplotClouds = 2  

       MatRplotPDF  = [1 2 3 4 35 36 40]; % MatRplotPDF(1,nbplotPDF) : plot the pdf of XX1,XX2,XX3,XX4,XX35,XX36,XX40 that is
                                          % QQ1,QQ2,QQ3,QQ4,QQ35,WW1,WW5 
                                          % plot, nbplotPDF = 7

       MatRplotPDF2D   = [1   2      % MatRplotPDF2D(nbplotPDF2D,2) : plot the joint pdf of XX1-XX2 that is QQ1-QQ2
    ...
    if ind_step4 == 1
       ...
       if ind_step4_task14 == 1
          ...
          Rmu = [1 2 3 4 5 6]';   % length of germ (Xi_1,...,Xi_mu)
          ...
          MatRplotHsamples = [];        % MatRplotHsamples(1,nbplotHsamples)
                                        % contains the components numbers of H_ar for which the plot of the realizations are made
          ...
          MatRplotHpdf = [1 2 3 4 5 6 7 8 9];            % MatRplotHpdf(1,nbplotHpdf)                                
          ...
          MatRplotHpdf2D = [1 2         % MatRplotHpdf2D(nbplotHpdf2D,2)
                            1 9];       % contains the 2 components numbers of H_d and H_ar for which the plot of the joint pdfs are made
