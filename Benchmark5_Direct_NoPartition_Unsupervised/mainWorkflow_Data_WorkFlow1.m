function [ind_step1,ind_step2,ind_step3,ind_step4,ind_step4_task13,ind_step4_task14,ind_step4_task15, ...
    ...
    ind_step3    = 1;      % 0: no step3 (post analysis: print and plot)
                           % 1:    step3  
    ind_step4    = 0;      % 0: no step4 (conditional statistics and polynomial chaos expansions)
                           % 1:    step4     
    ...
    if ind_step3 == 1
       ...
       %          Indx_obs = [Indq_obs
       %                      n_q + Indw_obs]
       %          In the example below, it is assumed that n_q = 28, n_w = 0, 
       %          Indq_obs = [2 3 4 5 7 10 15 20 26 27 28]', and Indw_obs = []
       %          then, Indx_obs = [2 3 4 5 7 10 15 20 26 27 28]

       MatRplotSamples = [];         % MatRplotSamples(1,nbplotSamples): plot samples of components of XX
                                     % MatRplotSamples = []: no plot, nbplotSamples = 0

       MatRplotClouds = [2   3  4    % MatRplotClouds(nbplotClouds,3): plot the clouds for XX2,XX3,XX4 
                         10 15 20    %                                 plot the clouds for XX10,XX15,XX20 
                         26 27 28];  %                                 plot the clouds for XX26,XX27,XX28 
                                     % plot nbplotClouds = 3 clouds 

       MatRplotPDF  = [2 3 4 10 15 20 26 27 28]; % MatRplotPDF(1,nbplotPDF) : plot the pdf of XX2,XX3,XX4,XX10,XX15,XX20,XX26,XX27,XX28,
                                                 % plot nbplotPDF = 9 pdfs

       MatRplotPDF2D   = [2   3      % MatRplotPDF2D(nbplotPDF2D,2) : plot the joint pdf of XX2-XX3
                          3   4      %                                plot the joint pdf of XX3-XX4 
                          10 15      %                                plot the joint pdf of XX10-XX15
                          15 20      %                                plot the joint pdf of XX15-XX20
                          27 28];    %                                plot the joint pdf of XX27-XX28
                                     % plot nbplotPDF2D = 5 joint pdfs
