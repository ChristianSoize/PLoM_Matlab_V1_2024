...
function [n_q,Indq_real,Indq_pos,n_w,Indw_real,Indw_pos,Indq_obs,Indw_obs,n_d,MatRxx_d] = mainWorkflow_Data_generation1
   ...
   %--- load data 
   %    Each component of X is a normalized Hermite polynomial
        % the dimension of the germ is 2
        % the maximum degree of the polynomial chaos is 6.    
        % the number of realizations in the training dataset is n_d = 400       
        % the components of XX are the polynomila chaos
        % the dimension n_x of XX is factorial(2 + 6)/(factorial(2)*factorial(6))) = 28 
        % including the multi_index (0,0))    
   ...
   Indq_obs = [2 3 4 5 7 10 15 20 26 27 28]';  % nq_obs component numbers of QQ that are observed , 1 <= nq_obs <= n_q
   Indw_obs = [];                              % nw_obs component numbers of WW that are observed,  1 <= nw_obs <= n_w
