...
function [ind_type_targ,Indq_targ_real,Indq_targ_pos,Indw_targ_real,Indw_targ_pos,N_r, ...
   ...
   load FileMatRxxTarget3 
        ...
        % --- ind_type_targ = 3: targets defined by the target mean value and the target covarianve matrix of the unscaled XX_targ 
        Rmeanxx_targ = Rmeanxx_targ_data(Indx_targ,1);  % Rmeanxx_targ(nx_targ,1): nx_targ components of mean value E{XX_targ} 
        MatRcovxx_temp = MatRcovxx_targ_data(Indx_targ,Indx_targ);  
        % Symmetrization
        MatRcovxx_temp = 0.5*(MatRcovxx_temp + MatRcovxx_temp');
        tol_rel        = 1e-10*max(diag(MatRcovxx_temp));
        MatRcovxx_targ = MatRcovxx_temp + tol_rel*eye(202,202);
   return
end
