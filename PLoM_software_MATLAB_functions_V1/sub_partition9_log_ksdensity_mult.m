function [Rlogpdf] = sub_partition9_log_ksdensity_mult(n,nsim,dim,nexp,MatRxData,MatRxExp)
   ...
   %
   %---OUTPUT
   %          Rlogpdf(nexp,1) = values of the log pdf at points  Rx^1,...,Rx^nexp      
   ...
   log_cons      = - sum(log(sqrt(2*pi)*sh*Rhatsigma));
   ...
   for alpha = 1:nexp    
       MatRbid(:,:,alpha) = shss*MatRxDataNorm(:,:)-repmat(MatRxExpNorm(:,alpha),1,nsim); % MatRxDataNorm(dim,nsim), MatRxExpNorm(dim,nexp)
   end
   ...
   Rlogpdf = log_cons + log((mean(MatRS,1))');                                            % Rlogpdf(nexp,1), MatRS(nsim,nexp) 
   return  
end
