...
function  [MatRx_obs] = sub_polynomialChaosQWU_PCAback(n_x,n_d,nu,n_ar,nx_obs,MatRx_d,MatReta_ar,Indx_obs,RmuPCA,MatRVectPCA, ...
   ...
   if nx_obs > n_x
      error('STOP1 in sub_polynomialChaosQWU_PCAback: nx_obs > n_x')
   end
   [nxtemp,ndtemp] = size(MatRx_d);                      %  MatRx_d(n_x,n_d) 
   if nxtemp ~= n_x || ndtemp ~= n_d
      error('STOP2 in sub_polynomialChaosQWU_PCAback: dimensions in MatRx_d(n_x,n_d) are not correct')
   end
   [nutemp,nartemp] = size(MatReta_ar);                  %  MatReta_ar(nu,n_ar) 
   if nutemp ~= nu || nartemp ~= n_ar
      error('STOP3 in sub_polynomialChaosQWU_PCAback: dimensions in MatReta_ar(nu,n_ar) are not correct')
   end
   nobstemp = size(Indx_obs,1);                         % Indx_obs(nx_obs,1) 
   if nobstemp ~= nx_obs 
      error('STOP4 in sub_polynomialChaosQWU_PCAback: the first dimension in Indx_obs must be nx_obs')
   end
   if length(Indx_obs) ~= length(unique(Indx_obs))
      error('STOP5 in sub_polynomialChaosQWU_PCAback: there are repetitions in Indx_obs')   % There are repetitions in Indx_obs
   end
   if any(Indx_obs < 1) || any(Indx_obs > n_x)                            % At least one  integer in Indx_obs is not within the valid range.
      error('STOP6 in sub_polynomialChaosQWU_PCAback: at least one  integer in Indx_obs is not in range [1,n_x] ')       
   end
