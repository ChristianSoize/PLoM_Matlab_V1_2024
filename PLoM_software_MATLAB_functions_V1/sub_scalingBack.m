function [MatRxx_obs] =  sub_scalingBack(nx_obs,n_x,n_ar,MatRx_obs,Indx_real,Indx_pos,Indx_obs,Rbeta_scale_real,Ralpha_scale_real, ...
   ...
   if ind_display_screen == 1                              
      disp('--- beginning Task11_ScalingBack')
   end

   if ind_print == 1
      fidlisting=fopen('listing.txt','a+');
      fprintf(fidlisting,'      \n '); 
      fprintf(fidlisting,' ------ Task11_ScalingBack \n ');
      fprintf(fidlisting,'      \n '); 
      fclose(fidlisting);  
   end
   ...
   if nx_obs > n_x
      error('STOP1 in sub_scalingBack: nx_obs > n_x')
   end
   [nobstemp,nartemp] = size(MatRx_obs);                %  MatRx_obs(nx_obs,n_ar) 
   if nobstemp ~= nx_obs || nartemp ~= n_ar
      error('STOP2 in sub_scalingBack: dimension errors in MatRx_obs(nx_obs,n_ar) ');
   end
   if length(Indx_obs) ~= length(unique(Indx_obs))
      error('STOP3 in sub_scalingBack: there are repetitions in Indx_obs');   % There are repetitions in Indx_obs
   end
   if any(Indx_obs < 1) || any(Indx_obs > n_x)
      error('STOP4 in sub_scalingBack: at least one  integer in Indx_obs is not within the valid range');  % At least one  integer in Indx_obs is not within the valid range.
   end
   ...
   if nbreal_obs + nbpos_obs ~= nx_obs
      error('STOP5 in sub_scalingBack1')  
   end
   ...
   if ind_print == 1
      ...
      fprintf(fidlisting,'      \n ');                                                                
      fprintf(fidlisting,'      \n '); 
      fprintf(fidlisting,' ------ Elapsed time for Task11_ScalingBack \n ');
      fprintf(fidlisting,'      \n '); 
