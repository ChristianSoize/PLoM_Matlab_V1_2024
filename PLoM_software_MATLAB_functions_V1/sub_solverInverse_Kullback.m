function [divKL] = sub_solverInverse_Kullback(MatRx,MatRy,ind_parallel)
    ...
    if n ~= nyTemp        
       error('STOP1 in sub_solverInverse_Kullback: dimension of X and Y must be the same')
    end
    if n <= 0 || Nx <= 0 || Ny <= 0
       error('STOP2 in sub_solverInverse_Kullback:  n <= 0 or Nx <= 0 or Nx <= 0');
    end
...
% [divKL]   = sub_solverInverse_Kullback(MatRx,MatRy,ind_parallel)
