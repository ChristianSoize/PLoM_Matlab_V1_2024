function sub_solverInverse_plot_Hd_Har(n_d,n_ar,MatReta_d,MatReta_ar,nbplotHsamples,nbplotHClouds,nbplotHpdf,nbplotHpdf2D, ...
   ...
   if nbplotHsamples >= 1 
      for iplot = 1:nbplotHsamples 
          ...
          numfig = numfig + 1;
          saveas(h,['figure_SolverInverse_',num2str(numfig),'_eta_ar_',num2str(ih),'.fig']);  
          close(h);
      end
   end                                                
   ...
   if nbplotHClouds >= 1  
      for iplot = 1:nbplotHClouds
          ...
          numfig = numfig+1;
          saveas(h,['figure_SolverInverse_',num2str(numfig),'_eta_clouds_',num2str(ih),'_',num2str(jh),'_',num2str(kh),'.fig']);  
          hold off
          close(h);
      end
   end
   ...
   if nbplotHpdf >= 1                                    
      ...
      for iplot = 1:nbplotHpdf
          ...
          numfig = numfig + 1;
          saveas(h,['figure_SolverInverse_',num2str(numfig),'_pdf_H_d_H_ar',num2str(ih),'.fig']);  
          hold off
          close(h);
      end
   end
   ...
   if nbplotHpdf2D >= 1
      ...
      for iplot = 1:nbplotHpdf2D
          ...
          title({['Joint pdf of $H_{\rm{d},',num2str(ih),'}$ with $H_{\rm{d},',num2str(jh),'}$ for $n_d = $',num2str(n_d)]}, ...
                  'FontWeight','Normal','FontSize',16,'Interpreter','latex');
          numfig = numfig + 1;
          saveas(h,['figure_SolverInverse_',num2str(numfig),'_joint_pdf_Hd',num2str(ih),'_Hd',num2str(jh),'.fig']); 
          close(h);
      end
   end
   ...
   if nbplotHpdf2D  > 0
      ...
      for iplot = 1:nbplotHpdf2D
          ...
          title({['Joint pdf of $H_{\rm{ar},',num2str(ih),'}$ with $H_{\rm{ar},',num2str(jh),'}$ for $n_{\rm{ar}} = $',num2str(n_ar)]}, ...
                  'FontWeight','Normal','FontSize',16,'Interpreter','latex');
          numfig = numfig + 1;
          saveas(h,['figure_SolverInverse_',num2str(numfig),'_joint_pdf_Har',num2str(ih),'_Har',num2str(jh),'.fig']); 
          close(h);
      end
   end
   return
